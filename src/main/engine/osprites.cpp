...
*/
    ...
    // Hide sprites that are fully outside the visible area. The right edge
    // uses >= permanently because the old comparison no longer works with the
    // widened x-coordinate used by widescreen rendering.
    if (     sprite_y2 < 256            /* fully off the top */
          || sprite_y1 > 479            /* fully off the bottom */
          || sprite_x2 < x2_bounds      /* fully off LHS */
          || sprite_x1 >= x1_bounds     /* fully off RHS */
       ) {
        hide_hwsprite(input, output);
        return;
    }
