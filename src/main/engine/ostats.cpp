...
void OStats::init_next_level_core()
{
    ...
    else if (outrun.game_state == GS_INGAME && oinitengine.checkpoint_marker)
    {
        ...
        ohud.draw_lap_timer(0x110554, stage_times[previous_stage], config.bugfix_checkpoint_lap_time()
            ? lap_ms[stage_times[previous_stage][2]]
            : ms_value);
